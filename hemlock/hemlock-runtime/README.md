...
![Runtime](https://img.shields.io/badge/runtime-Hemlock%20Gateway%20%2B%20Hemlock--loop%20%2B%20MCP-6f42c1)
...
A portable, self-hosted multi-agent runtime. The **Hemlock Gateway** owns the control plane
(identity, context, the agent loop); **Hemlock-loop** provides cognition (a per-agent "brain"
served over **MCP**); the two are bridged so each agent gets an isolated brain. The whole thing is built to
...
```
                        ┌─────────────────────────────────────────┐
   platforms  ───────▶  │  Hemlock Gateway  (control plane)       │
 (telegram/etc)         │  • identity + context injection         │
                        │  • runs the agent loop                  │
                        │  • LAZY-loads skills                     │
                        │            │  MCP handshake              │
                        │            ▼                             │
                        │  Hemlock-loop brain-<agent> (cognition) │
                        │  • per-agent, isolated                   │
...
- **One run-mode toggle — `HEMLOCK_MODE`** (`full` | `hermes` = brain-only | `openclaw` =
  gateway-only; the legacy value names are kept so existing configs never break).
- **Agent home — `HEMLOCK_HOME`** (canonical), with `HERMES_HOME` kept as a non-breaking alias.
- **Identity ownership:** the Hemlock Gateway injects `AGENTS.md` / `SOUL.md` / `TOOLS.md` /
  `IDENTITY.md` / `USER.md` / `HEARTBEAT.md` / `MEMORY.md`. The Hemlock-loop brain is cognition +
  data only — it never owns identity.
...
| **Per-agent brains** | `docker/hermes-agent/` | Isolated Hemlock-loop `brain-<name>` per agent over MCP, validated by a live handshake. |
...
namespaced as `<owner>-skills`; add sources from the menu's Skill Sources manager). All skills
pass skill-creator enterprise validation and are canonically maintained in a separate repo,
auto-committed there by a guardrail watcher on every version bump.
...
```
docker/        entrypoint, config generator, Hemlock-loop agent brain, self-healing daemons
...
**v1.0.0 — initial release.** Built and validated: run-mode toggle, per-agent brains + MCP
handshake, per-agent git volumes, the knowledge system, owner-driven backup/restore (incl.
Ventoy layer), memory discipline, the skill/guardrail toolchain, the provider-adaptive
skill-tag system (install-time remap + repo-side auto-strip; `AGENTS.md` §13), the 7-skill
kernel with auto-populating skill sources, the agent identity layer (constitution at t=0 +
identity hash), per-device config isolation, and the Hemlock Control dashboard served by the
gateway. Gateway port is **1437** (other engine installs on the host keep 18789 — no
collision). **In progress / not yet claimed done:** end-to-end autonomous crews,
semantic/vector memory. Component status is tracked honestly in `AGENTS.md` — if it isn't
listed as validated there, treat it as unverified.
