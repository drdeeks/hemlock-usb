...
CREWS_DIR = Path(os.environ.get("CREWS_DIR", str(ROOT_DIR / "crews")))
AGENTS_DIR = Path(os.environ.get("AGENTS_DIR", str(ROOT_DIR / "agents")))
PLUGINS_DIR = Path(os.environ.get("PLUGINS_DIR", str(ROOT_DIR / "plugins")))
...
def load_agents_manifest() -> Dict[str, Any]:
    ...
    if AGENTS_DIR.exists():
        # Load from agent directories (status in agent.json)
        skip_dirs = {"active", "archive", "templates", "rules", "envs", "workflow", "workspace-template"}
        for agent_dir in AGENTS_DIR.iterdir():
            ...
            if agent_json.exists():
                try:
                    ...
                    if status == 'active':
                        agents[agent.get('agent_id', agent_dir.name)] = agent
                except Exception as e:
                    print(f"Warning: Could not load {agent_json}: {e}")
        
        # Load from archive
        archive_dir = AGENTS_DIR / "archive"
        if archive_dir.exists():
            for arch_dir in archive_dir.iterdir():
                if not arch_dir.is_dir():
                    continue
                agent_json = arch_dir / "agent.json"
                if agent_json.exists():
                    try:
                        with open(agent_json, 'r') as f:
                            agent = json.load(f)
                        agents[agent.get('agent_id', arch_dir.name)] = agent
                    except Exception as e:
                        print(f"Warning: Could not load {agent_json}: {e}")
