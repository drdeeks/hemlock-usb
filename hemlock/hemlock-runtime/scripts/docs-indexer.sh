...
DOCS_DIR="${DOCS_DIR:-$RUNTIME_ROOT/docs}"
...
ensure_dirs() {
    mkdir -p "$REFERENCES_DIR" "$KNOWLEDGE_BASE_DIR" "$LOGS_DIR"
}
...
add_link() {
    ...
    # Add new link — values passed via argv (never interpolated into the source,
    # so a URL/title containing quotes, $(...), or newlines can't inject or corrupt).
    python3 -c '
import json, sys
from datetime import datetime

links_file, url, title, category = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4] or "general"
...
data["links"].append({
    ...
    "last_accessed": None, "indexed": False,
})
...
print("Added link: %s (%s)" % (title, url))
' "$LINKS_FILE" "$url" "$title" "$category"
...
try:
    ...
    exclude_patterns = ['node_modules', '__pycache__', '.git', '.secrets', '.hermes', '.archive', '*.log', '*.tmp', '*~', '*.enc', '*.key', '*.pem', '*secret*', '*password*', '*token*', '*api*key*']
...
search() {
    ...
    # Query passed via env (quoted heredoc = no shell interpolation → injection-safe).
    KB_QUERY="$query" INDEX_FILE="$INDEX_FILE" python3 <<'EOF'
import json
import os
import re
import sys
...
remove_link() {
    ...
    python3 -c '
import json, sys
links_file, url = sys.argv[1], sys.argv[2]
