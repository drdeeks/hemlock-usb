// Hemlock Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "hemlock-control-";
const EMBEDDED_CACHE_VERSION = "2026.6.11-e085fa1a3ffd";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__HEMLOCK_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Hemlock", body: event.data.text() };
  }

  const title = data.title || "Hemlock";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "hemlock-notification",
    data: { url: data.url || "./" },
  };
