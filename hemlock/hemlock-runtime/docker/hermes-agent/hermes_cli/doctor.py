...
def run_doctor(args):
    ...
    print(color("│                 🩺 Hemlock Doctor                       │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'hemlock-agent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'hemlock-agent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'hemlock-agent setup' to create one")
                issues.append("Run 'hemlock-agent setup' to create .env")
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'hemlock-agent setup' to migrate config")
                else:
                    issues.append("Run 'hemlock-agent doctor --fix' or 'hemlock-agent setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'hemlock-agent doctor --fix'")
        except Exception:
            pass
    ...
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memory"]
    ...
    memories_dir = hermes_home / "memory"
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'hemlock-agent doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'hemlock-agent setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'hemlock-agent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
