...
class GatewayRunner:
    ...
    async def _forward_to_mcp(self, event_type, data):
        """Forward events to MCP brain via Unix socket."""
        try:
            import socket
            import json
            
            sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            sock.connect("/tmp/agent_brain_mcp.sock")
            sock.sendall(json.dumps({"event": event_type, "data": data}).encode())
            sock.close()
        except Exception as e:
            self.logger.warning(f"MCP forward failed: {e}")

    async def pre_llm_call(self, messages):
        """Hook: Called before every LLM call."""
        await self._forward_to_mcp("pre_llm_call", {"messages": messages})
        return messages

    async def post_tool_call(self, tool_name, tool_args, result):
        """Hook: Called after every tool call."""
        await self._forward_to_mcp("post_tool_call", {
            "tool_name": tool_name,
            "tool_args": tool_args,
            "result": result
        })
            ...
            for prov_name, prov in providers_cfg.items():
                ...
                for m in models:
                    if isinstance(m, dict):
                        ...
                        mid = str(m)
                    marker2 = " <---" if "%s/%s" % (prov_name, mid) == "%s/%s" % (current_provider, current_model) else ""
                    lines.append("  - `%s`%s" % (mid, marker2))
                if not models:
