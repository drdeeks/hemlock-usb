...
def _log_memory_search(query: str, count: int) -> None:
    """Append a memory-search to the daily verification log. Fail-soft.

    Enables the discipline check described in AGENTS.md: after a subagent runs,
    the owner/agent can `cat logs/memory-searches-<date>.log` to confirm a search
    actually happened. Logs under $HEMLOCK_HOME (or legacy $HERMES_HOME).
    """
    try:
        import os as _os
        import datetime as _dt
        home = _os.environ.get("HEMLOCK_HOME") or _os.environ.get("HERMES_HOME")
        if not home:
            return
        log_dir = _os.path.join(home, "logs")
        _os.makedirs(log_dir, exist_ok=True)
        now = _dt.datetime.now()
        line = '%s | SEARCH | results=%3d | "%s"\n' % (
            now.isoformat(timespec="seconds"), int(count), (query or "").replace("\n", " ")
        )
        with open(_os.path.join(log_dir, "memory-searches-%s.log" % now.strftime("%Y-%m-%d")),
...
def session_search(
    ...
    try:
        ...
        _log_memory_search(query, len(summaries))
        return json.dumps({
            "success": True,
            "query": query,
            "results": summaries,
