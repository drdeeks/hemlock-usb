...
import { t as setupWizardCommand } from "./onboard-Ctqkmjxt.js";
...
async function createDefaultConfigIO() {
	const { createConfigIO } = await import("./io-B3FUbSMs.js");
...
async function resolveDefaultAgentWorkspaceDir(deps) {
	...
	const { DEFAULT_AGENT_WORKSPACE_DIR } = await import("./workspace-CxJ2PsVt.js");
	return DEFAULT_AGENT_WORKSPACE_DIR;
}
async function ensureDefaultAgentWorkspace(params) {
	const { ensureAgentWorkspace } = await import("./workspace-CxJ2PsVt.js");
	return ensureAgentWorkspace(params);
}
async function writeDefaultConfigFile(config) {
	const { writeConfigFile } = await import("./io-B3FUbSMs.js");
	await writeConfigFile(config);
}
async function formatDefaultConfigPath(configPath) {
	const { formatConfigPath } = await import("./logging-PQL_S1L_.js");
...
async function resolveDefaultSessionTranscriptsDir() {
	const { resolveSessionTranscriptsDir } = await import("./sessions-COjifoEk.js");
