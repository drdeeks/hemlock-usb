...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, T as getCommandPositionalsWithRootOptions, k as getVerboseFlag } from "./logger-CnaMaZPi.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-pzAQJGM1.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-BQJeivqd.js";
import { n as defaultRuntime } from "./runtime-Df11Xt6R.js";
import { a as enableConsoleCapture } from "./subsystem-D2gFnOcq.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-jndHmrmK.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BN73PDkq.js";
...
import { o as hasMemoryRuntime } from "./memory-state-CXPHU_bG.js";
import "./logging-B8ph7hAz.js";
import { n as ensureGlobalUndiciEnvProxyDispatcher } from "./undici-global-dispatcher-D8nttVY6.js";
import { i as initializeDebugProxyCapture, r as finalizeDebugProxyCapture } from "./runtime-BIHuBcHI.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-CR3m87sH.js";
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-BbCMxkzL.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-PkpnQOKu.js";
import { n as maybeWarnAboutDebugProxyCoverage } from "./coverage-CdfWfQm6.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-CKV1SPUT.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-IV1lDqHA.js");
			...
			const { statusCommand } = await import("./status-OYfjY--U.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-CbVr53Zo.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-BFdA6k64.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-Blvse8nt.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-F25il7dm.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-F25il7dm.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-CgXaAJRJ.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-CgXaAJRJ.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-BcCXmt_B.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-BrFrpGKZ.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-qbYphr7V.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-CrFKwlzk.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-BBRi3KTM.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-7s5z69bT.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-CDbtmJ_l.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-ecZ9oDzm.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { registerPluginCliCommandsFromValidatedConfig } = await import("./cli-CzV46n-y.js");
