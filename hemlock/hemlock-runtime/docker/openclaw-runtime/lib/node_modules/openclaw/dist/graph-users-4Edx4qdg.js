...
import { a as resolveMSTeamsCredentials } from "./token-DdRku1C2.js";
...
async function loadMSTeamsSdk() {
	const [appsModule, apiModule] = await Promise.all([import("./dist-H6jmsbKS.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)), import("./dist-CCecrUmk.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1))]);
	return {
		App: appsModule.App,
...
async function createBotFrameworkJwtValidator(creds) {
	const jwt = await import("./jsonwebtoken-vft5rjY5.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	const { JwksClient } = await import("./src-Bhf6KCiS.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
...
async function resolveGraphToken(cfg, options) {
	...
	if (options?.preferDelegated && msteamsCfg?.delegatedAuth?.enabled && creds.type === "secret") {
		const { resolveDelegatedAccessToken } = await import("./token-Co9i93AS.js");
...
//#endregion
export { inferPlaceholder as A, safeHostForUrl as B, ATTACHMENT_TAG_RE as C, encodeGraphShareId as D, applyAuthorizationHeaderForUrl as E, readNestedString as F, resolveAttachmentFetchPolicy as I, resolveMediaSsrfPolicy as L, isLikelyImageAttachment as M, isUrlAllowed as N, extractHtmlFromAttachment as O, normalizeContentType as P, resolveRequestUrl as R, isRevokedProxyError as S, IMG_SRC_RE as T, tryBuildGraphSharesUrlForSharedLink as V, buildUserAgent as _, fetchGraphJson as a, formatMSTeamsSendErrorHint as b, normalizeQuery as c, resolveGraphToken as d, readAccessToken as f, loadMSTeamsSdkWithAuth as g, createMSTeamsTokenProvider as h, fetchGraphAbsoluteUrl as i, isDownloadableAttachment as j, extractInlineImageCandidates as k, postGraphBetaJson as l, createMSTeamsAdapter as m, deleteGraphRequest as n, listChannelsForTeam as o, createBotFrameworkJwtValidator as p, escapeOData as r, listTeamsByName as s, searchGraphUsers as t, postGraphJson as u, ensureUserAgentHeader as v, GRAPH_ROOT as w, formatUnknownError as x, classifyMSTeamsSendError as y, safeFetchWithPolicy as z };
